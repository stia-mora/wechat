...
队列通过 PostgreSQL 行锁、任务去重、执行代次及租约支持多个 worker；同一发现服务仍建议只运行一个发现执行器。AI worker 可以独立部署。每分钟检查到期订阅，默认每 3 小时同步；历史未完成则继续回填，并检查最新一页，完成后从最新页做增量。默认自动任务每批 3 页、20 篇缺失正文，管理面板可以指定更小批次。
...
账号评分采用 `2026-09-evidence-v2`：固定为论证深度、来源透明度、信息增量、表达清晰度、观点审慎性。统一取最近 20 篇可读正文（每篇前 6000 字），0—5 分、0.5 步长，3 分合格、4 分优秀。每项至少引用两篇文章的逐字原文，5 分至少三篇；入库前核验文章 ID 和引文。缺证据显示“无法判断”，少于 20 篇或维度缺失不生成综合分。该评分不代表事实准确性、全网原创性或官方权威认证。

旧版自由维度分数退出展示和排名，画像其他内容仍保留。新标准同大类至少有 5 个可比较账号时显示相对位置，同分不计作被超越；不强制分布或人为压低评分。证据不匹配会反馈纠正一次，仍不匹配则删除无效引用、将对应维度标为无法判断并取消综合分；结构错误仍按失败处理。人工审核画像不会被自动重评覆盖。
